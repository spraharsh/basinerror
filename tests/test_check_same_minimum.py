...
def test_align():
    # print("hello world")
    specified_min = np.array([[1, 2, 3], [3, 4, 5]])
    ctol = 0.1
    dim = 3
    min_check = CheckSameMinimum(ctol, dim)
    min_check.minimalist = [specified_min]
    dummy_quench_result = specified_min + np.array([[0.1, 0.2, 0.3], [0, 0, 0]
                                                    ])
    aligned_quench = min_check.align_structures(specified_min,
                                                dummy_quench_result, 0)
    assert (np.all(
        aligned_quench == np.array([[1., 2., 3.], [2.9, 3.8, 4.7]])))


def test_check_same_structure_no_period():
    specified_min = np.array([[1, 2, 3], [2, 3, 4]])
    ctol_a = 0.5 - 0.1
    ctol_b = 0.5 + 0.1
    dim = 3
    ...
    dummy_quench_result = specified_min + np.array([[0.0, 0.3, 0.4], [0, 0, 0]
                                                    ])
    assert (min_check_a.check_same_structure(specified_min,
                                             dummy_quench_result) is False)
    assert (min_check_b.check_same_structure(specified_min,
                                             dummy_quench_result) is True)
...
def test_check_save_load():
    ...
    assert (np.all(
        res.initial_coords == np.array(min_check.initial_coords_list)))
...
def test_add_minimum():
    ...
    assert (np.all(
        min_check.minimalist[0] == np.array([[1., 0., 1.], [0., 1., 0.]])))
    assert (np.all(
        min_check.minimalist[1] == np.array([[1., 0.5, 1.5], [0., 1., 0.]])))
